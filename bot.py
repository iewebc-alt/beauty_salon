...
async def ignore_callback_handler(callback: types.CallbackQuery):
    await callback.answer("На этот день записи нет.", show_alert=True)
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "/my_appointments - Посмотреть или отменить мои записи\n"
...
async def show_my_appointments(message: types.Message):
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(f"{API_URL}/api/v1/clients/{telegram_user_id}/appointments")
        ...
        for appt in appointments:
            ...
            builder = InlineKeyboardBuilder()
            builder.button(text="❌ Отменить запись", callback_data=f"cancel_appt:{appt['id']}")
            await message.answer(response_text, reply_markup=builder.as_markup(), parse_mode="Markdown")
    except (httpx.RequestError, httpx.HTTPStatusError):
        await message.answer("😔 Не удалось загрузить список ваших записей.")
...
async def handle_contact(message: types.Message):
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.patch(f"{API_URL}/api/v1/clients/{telegram_user_id}", json=payload)
            response.raise_for_status()
...
async def cancel_handler(message: types.Message, state: FSMContext):
    ...
    if current_state is None: await message.answer("Нет активных действий для отмены."); return
    await state.clear()
...
@dp.callback_query(F.data == "back_to_master")
async def back_to_master_selection(callback: types.CallbackQuery, state: FSMContext):
...
async def service_selected(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback.message.edit_text("😔 Не удалось загрузить список мастеров.")
        await state.clear()
    await callback.answer()
...
async def process_date_selected(callback: types.CallbackQuery, state: FSMContext):
    ...
    if selected_date < date.today():
        await callback.answer("Нельзя выбрать прошедшую дату!", show_alert=True); return
    user_data = await state.get_data()
    ...
    try:
        ...
        await callback.message.edit_text("😔 Ошибка при поиске слотов.")
        await state.clear()
    await callback.answer()
