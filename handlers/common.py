...
async def cmd_start(message: types.Message, state: FSMContext):
    await state.clear()
    await message.answer(f"Здравствуйте, {message.from_user.full_name}! ✨\n" "Я — ваш виртуальный администратор салона «Элеганс». Рада помочь вам!\n\n" "Чем могу быть полезна?\n" "/book - Записаться на процедуру 💅\n" "/my_appointments - Посмотреть ваши записи 🗓️\n" "/cancel - Отменить текущее действие", reply_markup=types.ReplyKeyboardRemove())

@router.message(Command("cancel"))
async def cancel_handler(message: types.Message, state: FSMContext):
    current_state = await state.get_state()
    if current_state is None:
        await message.answer("Сейчас нет активного процесса, который можно было бы отменить. 😊"); return
    if current_state == AppointmentStates.awaiting_contact:
        await state.clear()
        await message.answer("Хорошо, понял(а) Вас. Ваша запись уже подтверждена. Если захотите ее отменить, воспользуйтесь командой /my_appointments. ✨", reply_markup=types.ReplyKeyboardRemove())
    else:
        await state.clear()
        await message.answer("Хорошо, я всё отменила. Давайте начнем заново, если хотите! /book", reply_markup=types.ReplyKeyboardRemove())
...
async def handle_contact_rejection(message: types.Message, state: FSMContext):
    text = message.text.lower(); negative_responses = ['нет', 'не', 'не хочу', 'отказ', 'позже']; question_responses = ['зачем', 'почему', 'для чего']
    ...
    if any(word in text for word in negative_responses):
        await message.answer("Хорошо, без проблем! Ваша запись уже подтверждена. До встречи в «Элеганс»! ✨", reply_markup=types.ReplyKeyboardRemove()); await state.clear()
    elif any(word in text for word in question_responses):
        await message.answer("Мы просим номер, чтобы администратор мог оперативно связаться с Вами в случае изменений в расписании мастера. 😊", reply_markup=keyboard)
    else:
        await message.answer("Я не совсем понял(а). Пожалуйста, либо поделитесь контактом с помощью кнопки, либо напишите 'нет'.", reply_markup=keyboard)
...
async def handle_unhandled_content(message: types.Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        gemini_response = await gemini_client.handle_natural_language(state=state, user_message=message.text, user_name=message.from_user.full_name, telegram_user_id=message.from_user.id)
            ...
            for call in tool_calls:
                ...
                if tool_name == 'create_appointment':
                    ...
                    try:
                        ...
                        dt_object = datetime.fromisoformat(api_response['start_time']); formatted_datetime = dt_object.strftime('%d %B в %H:%M')
                        success_messages.append(f"🎉 Отлично! Я успешно записал(а) Вас.\n\n" f"**Услуга:** {api_response['service_name']}\n" f"**Мастер:** {api_response['master_name']}\n" f"**Когда:** {formatted_datetime}\n\n" f"Будем ждать Вас в «Элеганс»!")
                        # --- 1. ОЧИЩАЕМ СОСТОЯНИЕ ПОСЛЕ УСПЕШНОЙ ЗАПИСИ ---
                        await state.clear()
                    ...
                    try:
                        ...
                        if not appointment_id:
                            success_messages.append(f"⚠️ Не удалось определить ID для отмены.")
                            continue
                        data = await state.get_data()
                        cancellation_cache = data.get("cancellation_cache", {})
                        appt_info = cancellation_cache.get(str(appointment_id))
                        await api_client.delete_appointment(appointment_id)
                        if appt_info:
                            dt_object = datetime.fromisoformat(appt_info['start_time'])
                            formatted_datetime = dt_object.strftime('%d %B %Y в %H:%M')
                            success_messages.append(f"Готово! Ваша запись на услугу:\n" f"✨ **{appt_info['service_name']}** к мастеру **{appt_info['master_name']}**\n" f"🗓️ на **{formatted_datetime}**\n" f"успешно отменена.")
                        else:
                            success_messages.append(f"✅ Запись с ID {appointment_id} успешно отменена.")
                    except (httpx.RequestError, httpx.HTTPStatusError):
                        success_messages.append(f"❌ Не удалось отменить запись с ID {tool_args.get('appointment_id')} из-за технической ошибки.")
            
            if success_messages:
                await message.answer("\n\n".join(success_messages), parse_mode="Markdown")
                # --- 2. ОЧИЩАЕМ СОСТОЯНИЕ ПОСЛЕ УСПЕШНОЙ ОТМЕНЫ ---
                await state.clear()

    except Exception as e:
        logging.error(f"Критическая ошибка в хендлере: {e}")
        if msg: await bot.edit_message_text(text="😔 Простите, в боте произошла критическая ошибка. Мы уже работаем над этим.", chat_id=message.chat.id, message_id=msg.message_id)
