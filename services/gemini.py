...
# ... (определение инструментов остается без изменений) ...
get_salon_info_func = FunctionDeclaration(
    name="get_salon_info",
    description="Получает актуальный список всех услуг, цен, мастеров и их специализаций. Используй это, если клиент спрашивает 'что вы умеете?', 'какие цены?' или если ты не уверен в названии услуги.",
    parameters={"type": "OBJECT", "properties": {}},
)
check_availability_func = FunctionDeclaration(
    name="check_availability",
    description="Проверяет свободные слоты для конкретной услуги на конкретную дату. Обязательно используй этот инструмент перед тем, как предлагать клиенту время.",
    parameters={
        "type": "OBJECT",
        "properties": {
            "service_name": {"type": "STRING", "description": "Точное название услуги, например 'Женская стрижка'."},
            "appointment_date": {"type": "STRING", "description": f"Дата для проверки в формате YYYY-MM-DD. Сегодня: {date.today().isoformat()}."},
        },
...
create_appointment_func = FunctionDeclaration(
    ...
    description="Финальное действие. Создает запись в календаре. Используй ТОЛЬКО ПОСЛЕ того, как ты проверил наличие свободных слотов через `check_availability` и клиент подтвердил конкретное время.",
    parameters={
        "type": "OBJECT",
        "properties": {
            "service_name": {"type": "STRING", "description": "Название услуги."},
            "appointment_date": {"type": "STRING", "description": "Дата записи в формате YYYY-MM-DD."},
            "appointment_time": {"type": "STRING", "description": "Время записи в формате HH:MM."},
            "master_name": {"type": "STRING", "description": "Имя мастера, если оно известно."},
        },
    ...
    def __init__(self, api_key: str):
        ...
        if not api_key:
            ...
            try:
                ...
                system_instruction = (
                    "Ты — 'Элеганс-Агент', умный ИИ-администратор. Твоя задача — помочь клиенту записаться, используя инструменты. "
                    "Твой рабочий процесс: "
                    "1. Если нужно, используй `get_salon_info`, чтобы узнать об услугах. "
                    "2. Когда клиент выберет услугу и дату, ОБЯЗАТЕЛЬНО используй `check_availability`, чтобы найти свободные слоты. "
                    "3. Предложи найденные слоты клиенту. "
                    "4. Когда клиент выберет конкретное время, используй `create_appointment` для записи. "
                    "Не придумывай информацию, всегда получай ее через инструменты."
                )
            ...
            for _ in range(5):
                ...
                if response_part.function_call:
                    ...
                    if tool_name == "get_salon_info":
                        ...
                        if not api_result:
                            tool_response_content = "На эту дату свободных слотов нет. Предложи клиенту выбрать другую дату."
                        else:
                    ...
                    # --- 2. ИСПРАВЛЕНИЕ ЗДЕСЬ ---
                    # Создаем объект ответа инструмента через словарь, а не через класс Part
                    tool_response_part = {
                        "function_response": {
                            "name": tool_name,
